...
export class PerroComponent implements OnInit, AfterViewInit {
  ...
  obtenerPerro() {
    ...
    this._PerroServicio.listaPaginada(this.page, this.pageSize, this.searchTerm).subscribe({
      next: (data) => {
        if (data && data.data && data.data.length > 0) {

          console.log(data.data)
          // data.data.forEach((Perro: Perro) => {
          //   if (Perro.imageUrl) {
          //     // Perro.imageData = this._usuarioServicio.decodeBase64ToImageUrl(Perro.imageData);
          //     console.log('Imagen URL:', Perro.imageUrl); // Verifica si la URL está correcta
          //     Perro.imageUrl = Perro.imageUrl;
          //   }
          // });
          this.totalPerros = data.total;
          this.totalPages = Math.ceil(this.totalPerros / this.pageSize);
          this.dataListaPerros = data.data;
        } else {
          ...
          this.dataListaPerros.data = []; // Limpia los datos existentes
          // Swal.fire({
          //   icon: 'warning',
          //   title: 'Advertencia',
